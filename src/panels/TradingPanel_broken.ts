...
export class TradingPanel {
  ...
  async handleCallback(ctx: Context): Promise<void> {
    ...
    if (!state) {
      await ctx.reply('⚠️ Panel not found. Please send a token address to open a new panel.');
      return;
    }
  ...
  private async executeBuy(state: UserPanelState): Promise<void> {
    ...
    const quote = await this.tradeRouter.getQuote({
      inputMint: 'So11111111111111111111111111111111111111111112',
      outputMint: token_address,
      amount: action_data.selected_amount * 1_000_000_000,
      slippageBps: action_data.slippage * 100,
    });
    ...
    const txSignature = await this.tradeRouter.executeSwap(
      wallet,
      quote,
      action_data.slippage
    );

    const price = state.token_data.current_price;
    const amount = action_data.selected_amount / price;
    await this.positionTracker.updatePosition(
      state.user_id,
      token_address,
      price,
      amount,
  ...
  private async executeSell(state: UserPanelState): Promise<void> {
    ...
    const quote = await this.tradeRouter.getQuote({
      ...
      outputMint: 'So11111111111111111111111111111111111111111112',
      amount: action_data.selected_amount,
      slippageBps: action_data.slippage * 100,
    });
    ...
    await this.positionTracker.closePosition(
      ...
      action_data.selected_amount,
      txSignature
    );

    const positionId = `${state.user_id}_${token_address}`;
    await this.tpslManager.cancelRelatedOrders(positionId);
  ...
  private async cancelLimitOrder(state: UserPanelState): Promise<void> {
    const positionId = `${state.user_id}_${state.token_address}`;
  ...
  private async closePanel(state: UserPanelState): Promise<void> {
    state.closed = true;
    this.stateManager.setState(state.user_id, state);
    this.autoRefreshService.stopAutoRefresh(state.user_id);
    
    console.log(`[TradingPanel] Panel closed for user ${state.user_id}`);
  }

  private async updatePanelMessage(state: UserPanelState): Promise<void> {
    try {
      await this.bot.telegram.editMessageText(
        state.user_id,
        state.message_id,
        undefined,
        this.generatePanelText(state),
        {
          parse_mode: 'Markdown',
          reply_markup: {
            inline_keyboard: this.generateKeyboard(state),
          },
        }
      );
    } catch (error) {
